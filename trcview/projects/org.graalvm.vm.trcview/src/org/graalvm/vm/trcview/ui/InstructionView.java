...
public class InstructionView extends JPanel {
    ...
    private static String getValue(MemoryEvent access, StepFormat fmt) {
        ...
        if (fmt.numberfmt == StepFormat.NUMBERFMT_OCT) {
            ...
            if (access.hasData()) {
                ...
                switch (access.getSize()) {
                    case 1:
                        ...
                        val.append(OctFormatter.tooct(access.getValue(), 3));
                        break;
                    case 2:
                        str = Stringify.i16((short) access.getValue());
                        val.append(OctFormatter.tooct(access.getValue(), 6));
                        break;
                    case 4:
                        str = Stringify.i32((int) access.getValue());
                        val.append(OctFormatter.tooct(access.getValue(), 11));
                        break;
                    case 8:
                        str = Stringify.i64(access.getValue());
                        val.append(OctFormatter.tooct(access.getValue(), 22));
                        break;
                    case 16:
                        str = Stringify.i128(access.getVector());
                        val.append(OctFormatter.tooct(access.getVector().getI64(0), 22));
                        val.append(" ");
                        val.append(OctFormatter.tooct(access.getVector().getI64(1), 22));
                        break;
                }
                if (str != null) {
            ...
            if (access.hasData()) {
                ...
                switch (access.getSize()) {
                    case 1:
                        str = Stringify.i8((byte) access.getValue());
                        val.append(HexFormatter.tohex(access.getValue(), 2));
                        break;
                    case 2:
                        str = Stringify.i16((short) access.getValue());
                        val.append(HexFormatter.tohex(access.getValue(), 4));
                        break;
                    case 4:
                        str = Stringify.i32((int) access.getValue());
                        val.append(HexFormatter.tohex(access.getValue(), 8));
                        break;
                    case 8:
                        str = Stringify.i64(access.getValue());
                        val.append(HexFormatter.tohex(access.getValue(), 16));
                        break;
                    case 16:
                        str = Stringify.i128(access.getVector());
                        val.append(HexFormatter.tohex(access.getVector().getI64(0), 16));
                        val.append(" ");
                        val.append(HexFormatter.tohex(access.getVector().getI64(1), 16));
                        break;
                }
                if (str != null) {
                    val.append(" ['").append(str).append("']");
    ...
    private static String decode(MemoryEvent access, Variable var, StepFormat fmt) {
        Type type = var.getType();
        String name = var.getName(fmt);
        if (type == null && name != null && var.getAddress() == access.getAddress()) {
            return name + " = " + getValue(access, fmt);
            ...
            if (type.getElements() > 1) {
                ...
                if (off == 0) {
                    return name + "[" + idx + "] = " + getValue(access, fmt);
                } else {
                    return null;
                }
    ...
    private String decode(MemoryEvent access, TypedMemory mem, StepFormat fmt) {
        long addr = access.getAddress();
        Variable var = mem.get(addr);
        if (var != null) {
            return decode(access, var, fmt);
        } else {
            Symbol sym = trc.getSymbol(addr);
            if (sym != null && sym.getValue() == addr && sym.getName().length() > 0) {
                return sym.getName() + " = " + getValue(access, fmt);
            }
        }
    ...
    private String autocomment(StepEvent step) {
        ...
        if (!reads.isEmpty()) {
            for (MemoryEvent read : reads) {
                // process
                String result = decode(read, mem, fmt);
                if (result != null) {
                    return result;
                }
            }
        ...
        if (!writes.isEmpty()) {
            for (MemoryEvent write : writes) {
                // process
                String result = decode(write, mem, fmt);
                if (result != null) {
                    return result;
                }
            }
