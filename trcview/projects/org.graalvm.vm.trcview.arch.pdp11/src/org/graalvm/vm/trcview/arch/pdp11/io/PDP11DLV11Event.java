...
public class PDP11DLV11Event extends DeviceEvent {
    ...
    public IoEvent getIoEvent() {
        if (type == DLV11_RX) {
            return new IoEvent(getTid(), step, channel, true, Character.toString((char) (value & 0xFF)));
        } else if (type == DLV11_TX) {
            return new IoEvent(getTid(), step, channel, false, Character.toString((char) (value & 0xFF)));
        } else {
            return null;
        }
    }
